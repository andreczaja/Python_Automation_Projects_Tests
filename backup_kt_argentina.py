...
def existe_registro(task_name, data):
    df_existe_registro = df_main[(df_main['task_name'] == task_name) & (df_main['data'] == data)]
...
def obter_tarefas_status_especifico(data, task_name, status_kt, status_dtp):
    return pd.read_sql_query("SELECT task_name FROM table_kt WHERE data = ? AND status_kt = ? AND status_dtp = ?",
        conn,
        params=(data, task_name, status_kt, status_dtp)
        )
